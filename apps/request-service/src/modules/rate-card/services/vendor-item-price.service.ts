...
export class VendorItemPriceService {
  private readonly logger = new AppLogger(VendorItemPriceService.name);
  ...
  async getActiveMapping(fk_vendor_id: string, fk_item_id: string) {
    try {
      ...
      return ResponseHelper.success(mapping, 'Active price mapping fetched successfully');
    } catch (error: any) {
   ...
   */
  async rebuildForVendor(fk_vendor_id: string) {
    try {
      const prisma = await this.getSchemaClient();
      ...
      return ResponseHelper.success(null, 'Vendor item price mappings rebuilt successfully');
    } catch (error: any) {
      this.logger.error(error.stack || error.message);
      return ResponseHelper.error(error.message);
    }
   ...
   */
  async deactivateForRateCard(fk_rate_card_id: string) {
    try {
      const prisma = await this.getSchemaClient();
      ...
      return ResponseHelper.success(null, 'Vendor item price mappings deactivated successfully');
    } catch (error: any) {
      this.logger.error(error.stack || error.message);
      return ResponseHelper.error(error.message);
    }
